...
const NowPlayingMovie = () => {
  ...
  const getMovie = async() => {
        const respon = await axios.get('https://api.themoviedb.org/3/movie/now_playing?api_key=6dcaf0667fe77633ed4db188ddc50fa2&language=en-US&page=1')
        .then(function(response){
            // console.log('asdasd',response.data.results)    
  ...
  return (
    <>
     <Text style={{fontSize:26, fontWeight:'700', marginVertical:10, marginLeft:10}}>Now Playing Movie</Text>
     <View style={{flexDirection:'row', justifyContent: "space-between", margin:2,
     flexWrap: "wrap",}}>
 {data.map((list, index) => {
     return(
     <TouchableOpacity onPress={() => navigation.navigate("Detail", {itemId:list.id})} style={{backgroundColor:'white',flexBasis: '47%',marginHorizontal:5, marginVertical:8, borderTopLeftRadius:8, borderTopRightRadius:8, borderBottomLeftRadius:8, borderBottomRightRadius:8}}>
